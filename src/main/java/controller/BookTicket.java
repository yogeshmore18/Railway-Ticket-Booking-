...
public class BookTicket extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (user == null) {
			res.getWriter().print("<h1 style='color:red'>Session Expired Login Again</h1>");
			...
			int numberOfSeats = Integer.parseInt(req.getParameter("seat"));
			Date doj = Date.valueOf(req.getParameter("date"));
			Date dob = Date.valueOf(LocalDate.now());
			if (numberOfSeats <= 0) {
				res.getWriter().print("<h1>Seat can not be less than 1</h1>");
				req.getRequestDispatcher("UserHome.html").include(req, res);
			} else {
				if (from.equals(to)) {
					res.getWriter().print("<h1>Source and Destination can not be same</h1>");
					req.getRequestDispatcher("UserHome.html").include(req, res);
				} else {
					int fromPos = 0;
					int toPos = 0;
					...
					if (fromPos > toPos) {
						res.getWriter().print("<h1>Select Valid Source and Destination</h1>");
						req.getRequestDispatcher("UserHome.html").include(req, res);
					} else {
						...
						if (Period.between(dob.toLocalDate(), doj.toLocalDate()).getDays() < 0 || flag) {
							res.getWriter().print("<h1>Train Not Available in the selected date </h1>");
							req.getRequestDispatcher("UserHome.html").include(req, res);
						} else {
							if (user.getWallet() < amount) {
								res.getWriter().print("<h1>Insufficient funds for booking ticket</h1>");
								req.getRequestDispatcher("UserHome.html").include(req, res);
							} else {
								if (train.getSeat() < numberOfSeats) {
									res.getWriter().print("<h1>Seats not Available</h1>");
									req.getRequestDispatcher("UserHome.html").include(req, res	);
								} else {
									...
									res.getWriter().print("<h1>Ticket Booked Successfully</h1>");
									req.getRequestDispatcher("Home.html").include(req, res);
								}
							}
